        ...
        <div className="z-10 flex min-h-screen flex-1 flex-col items-center justify-center pt-24">
            <main
                ...
                <div
                    ...
                    <StyledText
                        text={<p>Ein Battle-Rap zwischen Donald Trump und Joe Biden, widersprüchliche Weihnachtsgrüße der Queen oder Donald Trump,
                            ...
                            überhaupt treffen und ein mögliches Ende ihre Karriere riskieren? <br/>Die Antwort darauf lässt sich einfach sagen: gar nicht.
                            ...
                            im Netz komplett verändert. <br/><br/>Es stellt sich die Frage: wie funktionieren Deepfakes? <br/>Antwort: Neuronale Netze, bzw. Machine
                            Learning (deutsch: maschinelles Lernen).</p>}/>
                    <SubHeader title={"Neuronale Netze"}/>
                    <StyledText
                        text={<p className={"text-left text-white m-5 whitespace-pre-wrap"}>Wie der Name Deepfake bereits zu verstehen gibt, handelt es sich bei dem Wort um
                            ...
                            des Deep Learnings, genauer gesagt, künstlichen Neuronalen Netzen.</p>}/>
                    <img src={"/animationDF.png"}/>
                    <StyledText text={<p>Deep Learning gehört in der Informatik zum Bereich der Künstlichen
                        Intelligenz und ist wiederum ein Teilbereich des sogenannten Machine Learnings (dt. Maschinelles Lernen). <br/>Ziel des Machine Learnings,
                        ...
                        ist hierbei nötig. [2] <br/>Statt Systeme anhand von vorhandenen Daten trainieren zu lassen, wird beim Deep Learning das menschliche Gehirn
                        ...
                        würde: betrachten, analysieren und Schlussfolgerung ziehen [3]. <br/>Große Datensätze werden schneller erfasst und analysiert als ein Mensch
                        ...
                        Einschreiten in den Lernprozess von Nöten ist [2]</p>}/>
                    <img src={"/zwiebel.png"}/>
                    <StyledText
                        text={<p>Um die Funktionsweise eines künstlichen neuronalen Netzes (kurz KNN) besser nachvollziehen zu können und somit den gesamten
                            ...
                            Ein künstliches neuronales Netz besteht aus der Eingabeschicht, den mehreren verborgenen Schichten und der Ausgabeschicht.</p>}/>
                    <img className={"p-4"} src={"/neuronalesNetz.png"}/>
                    <StyledText
                        text={<p>Ein Computer soll nun mithilfe von einem künstlichen neuronalen Netz eine handgeschriebene Zahl erkennen. Zu Beginn werden
                            aus der Zahl gewisse Grundwerte entnommen und in die <span className={"text-lime-400"}>Eingabeschicht</span> aufgenommen. In unserem Fall wird die handgeschriebene
                            Notiz in einzelne Pixel aufgeteilt und diese in die Neuronen der <span className={"text-lime-400"}>Eingabeschicht</span> geworfen. <br/>Ein Neuron enthält somit ein Pixel.<br/>
                            ...
                            trifft. <br/>Generell gilt, je höher die Gewichtung, desto repräsentativer ist die zugehörige Verbindung für das Endergebnis. Weist eine
                            Gewichtung den Wert Null auf, so hat das dazugehörige Neuron keinen Einfluss auf das Endergebnis. Diese gewichteten Daten
                            werden nun von der <span className={"text-lime-400"}>Eingabeschicht</span> in die <span className={"text-sky-400"}>verborgenen Schichten</span> weitergegeben.<br/>Die <span className={"text-sky-400"}>verborgenen Schichten</span> befinden sich zwischen der
                             <span className={"text-lime-400"}>Eingabe-</span> und der <span className={"text-yellow-300"}>Ausgabeschicht</span>. Während die <span className={"text-lime-400"}>Eingabe-</span> oder <span className={"text-yellow-300"}>Ausgabeschicht</span> aus jeweils einer Ebene besteht, gibt es bei den
                            <span className={"text-sky-400"}>verborgenen Schichten</span> beliebig viele Ebenen [4]. In diesen <span className={"text-sky-400"}>verborgenen Schichten</span> findet der eigentliche Lern- bzw. Trainingsprozess
                            statt. <br/>In unserem Beispiel lassen sich hier die Kombination von mehreren Pixeln im Verhältnis zueinander identifizieren, Kanten
                            ...
                            Name bereits zu erkennen gibt, nicht sichtbar. Ganz im Gegensatz zu der <span className={"text-lime-400"}>Ein-</span> und <span className={"text-yellow-300"}>Ausgabeschicht</span>, wo diese erkennbar sind.
                            <br/>Die <span className={"text-yellow-300"}>Ausgabeschicht</span> ist die letzte Schicht in unserem künstlichen Neuronalen Netz und hängt unmittelbar an die letzte Ebene
                            ...
                            hervor. <br/>Diese Verbindung repräsentiert das Endergebnis und das IT-System weiß, dass die handgeschriebene Notiz zu Beginn die Zahl 6 ist.</p>}/>
                    <SubHeader title={"Topologien neuronaler Netze"}/>
                    <StyledText
                        text={<p>Neuronale Netze weisen unterschiedliche Strukturen, auch Topologie genannt, auf. Diese beschreibt, wie das künstliche neuronale
                            Netz aufgebaut ist und wie viele Neuronen sich auf wie vielen Schichten befinden. <span className={"text-heinzBlau"}>Die bekanntesten Topologien bei neuronalen
                                Netzen sind das einschichtige Feedforward-Netz, das mehrschichtige Feedforward-Netz und das rekurrente Netz.</span> <br/>Bei einschichtigen
                            ...
                            sich selbst verbinden. [5]</p>}/>
                    <img className={"p-4"} src={"/topologie.png"}/>
                    <SubHeader title={"Mathematik"}/>
                    <SubHeader title={"Einsatz neuronaler Netze bei Deepfakes"}/>
                    <StyledText
                        text={<p>Bei Deepfakes wird der Prozess des Deep Learnings besonders bei der Gesichtserkennung und der Angleichung von dem
                            Ausgangsgesicht an die Mimik des Zielgesichts verwendet. Deepfake-Algorithmen basieren auf der Idee von <span className={"text-heinzBlau"}>„Generative Adversarial Networks“</span>,
                            also zwei bilderzeugenden neuronalen Netzwerken, welche sich gegenüberstehen.<br/>So werden zunächst die zwei Ausgangsvideos, in ihre Einzelbilder
                            ...
                            Gewichtungen modifiziert. <br/>Der Vorgang wird nun so lange wiederholt, bis die Ausgabe der Decoder dem Ursprungsbild stark ähnelt. Dies wird auch
                            Überwachtes Lernen genannt.</p>}/>
                    <img className={"p-4"} src={"/algorithmus.png"}/>
                    <StyledText
                        text={<p>Anschließend findet der eigentliche Gesichter Tausch statt, indem der Encoder des einen Gesichtes an den Decoder des anderen Gesichtes angeschlossen wird. [7]</p>}/>
                    <img className={"p-4"} src={"/algorithmus2.png"}/>
                    <StyledText text={<p>Reales Beispiel (Heinz und Teammitglied Jonas):</p>}/>
                    ...
                    <StyledText
                        text={<p>Neben dem Gesichtstausch gibt es natürlich noch weitere Anwendungsmöglichkeiten.</p>}/>
